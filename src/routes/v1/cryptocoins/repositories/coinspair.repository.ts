...
import { PaginatedCoinsPairInterface, PaginatedCryptocoinsInterface } from "@interfaces/paginated-users.interface";
...
export class CoinsPairRepository {
    constructor(
        @InjectModel(CoinsPair.name) private coinsPairModel: Model<CoinsPairDocument>,
    ...
    public async create(coinsPair: CoinsPair) {
        try {
            ...
            this.logger.log(`Successfully created a new coins pair in db ${newCoinsPair}`);
            return newCoinsPair.toObject();

        } catch (error) {
            this.logger.error(`Unexpected error while creating new coins Pair ${JSON.stringify(coinsPair)} due to ${error.message}`);
    ...
    public async getById(id: Types.ObjectId): Promise<CoinsPair | null> {
        try {
            ...
            this.logger.log(`Successfully found coins Pair with id ${coinsPair._id} from db`);
            return coinsPair
        } catch (error) {
            this.logger.error(`Unexpected error while searching coins pair with id ${id} due to ${error.message}`)
            throw new MongoError(error);
        }
    }
    ...
    public async getBySymbol(symbolPair: string): Promise<CoinsPair | null> {
        try {
            ...
            this.logger.log(`Successfully found crypto coin pair in db ${coinsPair.symbolPair}`);
            return coinsPair
        } catch (error) {
            this.logger.error(`Unexpected error while searching for cryptocoin ${symbolPair} due to ${error.message}`)
            throw new MongoError(error);
        }
    }
    ...
    public async getByQuery(query: UpdateCoinPairDto): Promise<CoinsPair | null> {
        try {
            ...
            const coinsPair: CoinPair = await this.coinsPairModel
                .findOne(customQuery)
                .lean();
            this.logger.log(`Successfully found crypto coin pair in db ${query.baseSymbol} and ${query.quoteSymbol}`);
            return coinsPair
        } catch (error) {
            this.logger.error(`Unexpected error while searching for coin pair ${query.baseSymbol} and ${query.quoteSymbol} due to ${error.message}`)
            throw new MongoError(error);
        }
    }

    public async getAllCoinsPairWithPagination(options: PaginationParamsInterface): Promise<PaginatedCoinsPairInterface> {
        try {

            const [coinPairs, totalCount] = await Promise.all([
                this.coinsPairModel.find()
                    .limit(PaginationUtil.getLimitCount(options.limit))
                    .skip(PaginationUtil.getSkipCount(options.page, options.limit))
                    .lean(),
                this.coinsPairModel.count()
                    .lean(),
            ]);

            this.logger.log(`Successfully found the list of coinsPair from db ${totalCount}`)
