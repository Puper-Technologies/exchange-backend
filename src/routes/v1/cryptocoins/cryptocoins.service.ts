...
export class CryptocoinsService {
  ...
  constructor(private readonly cryptocoinRepository: CryptocoinRepository, private readonly coinsPairRepository: CoinsPairRepository, private logger: MyLogger, private httpAxiosClient: HttpService) {
    this.logger.setContext(CryptocoinsService.name)
  }

  async addCryptoCurrency(createCryptocoinDto: Partial<Cryptocoin>): Promise<Cryptocoin> {
  ...
  async addExchangeCryptoCurrencies(cryptoExchange: string, quoteSymbol: string, cmcApiKey: string) {
    try {

      // Api to fetch all coin details from respective exchange
      const cryptoAssetList$ = await this.httpAxiosClient.get(
        CRYPTO_DATA[cryptoExchange].BASE_URL +
        CRYPTO_DATA[cryptoExchange].EXCHANGE_INFO,
      ).pipe(map((x) => x.data),
        catchError(e => {
          this.logger.error(`Error in accessing ${cryptoExchange} exchange supported coin info ${JSON.stringify(e.response.data.msg)} status code ${e.response.status}`)
      ...
      const cryptoAssetCMCDetails$ = await this.httpAxiosClient.get(
        ...
        catchError(e => {
          this.logger.debug(`Error in accessing CMC api ${JSON.stringify(e.response.data.msg)} status code ${e.response.status}`)
          throw new InternalServerErrorException(e.response.data.msg, e.response.status);
        }))

      const exchangeSupportedCryptoList = await lastValueFrom(cryptoAssetList$);
      const cmcCryptoCurrenciesData = await lastValueFrom(cryptoAssetCMCDetails$)
      const supportingCoinsId = [];
      const supportedQuoteCurrencies = lodash.filter(exchangeSupportedCryptoList.symbols, { quoteAsset: quoteSymbol });
      ...
      this.logger.debug(`Length of supported found crypto currencies on ${cryptoExchange}`, cmcCryptoCurrenciesData.data.cryptoCurrencyList.length);

      const currenciesMetaInfo = supportedQuoteCurrencies.map((coin) => {
        let cmcCoin: any = lodash.find(cmcCryptoCurrenciesData.data.cryptoCurrencyList, { symbol: coin.baseAsset }) || null
      ...
      const cmcAssetInfo$ = await this.httpAxiosClient.get(
        `https://pro-api.coinmarketcap.com/v2/cryptocurrency/info?id=${supportingCoinsId.toString()}`,
        {
          headers: {
            "X-CMC_PRO_API_KEY": (cmcApiKey) ? cmcApiKey : "458f48ee-d81e-4d2a-8484-02306834ecb1"
          }
        }
      ).pipe(map((x) => x.data),
        catchError(e => {
          this.logger.error(`Error Occured while accessing CMC api ${JSON.stringify(e.response.data.msg)} status code ${e.response.status}`)
          throw new InternalServerErrorException(e.response.data.msg, e.response.status);
        }))
      ...
      for (const coin of coinDetails) {
        addedCoins.push(await this.cryptocoinRepository.addExchangeCurrencies(coin));
      }
      this.logger.debug(`Success fully added data:`, addedCoins.length)
      // const exchangeCurrencies = ;
      return addedCoins.length;
    } catch (error) {
      throw new InternalServerErrorException(`Unexpected error in getExchangeInfo method while fetching exchange ${Platform} info due to : ${error.message}`);
    }
  ...
  async findAllCryptoCoins(
    ...
    if (search)
      query['$or'] = [{ name: { $regex: search, $options: 'i' } }, { symbol: { $regex: search, $options: 'i' } }]

    try {
      let coinsList = await this.cryptocoinRepository.getAllCryptocoinsWithPagination(query, options);
      let results: Cryptocoin[] = await Promise.all(coinsList.paginatedResult.map(async (cryptoCoin): Promise<Cryptocoin> => {
      ...
      console.log("coin list", quoteSymbol, coinsList.paginatedResult[0], results[0]);
      return coinsList;
    } catch (error) {
      throw new NotAcceptableException(`Error occurred in cryptocoins service ${error.message}`)
    }
  }
  async getCurrentPriceOfCoin(cryptoExchange: string, baseSymbol: string, quoteSymbol: string) {
  ...
  async addCoinsPair(createCryptocoinPairDto: CreateCryptocoinPairDto) {
    try {
      const cryptoCoin = await this.cryptocoinRepository.getBySymbol(createCryptocoinPairDto.baseSymbol) || null;
      this.logger.debug(`Crypto coin details in service ${JSON.stringify(cryptoCoin)}`)
      if (lodash.isEmpty(cryptoCoin)) {
        throw new NotFoundException(`Not found crypto coin with symbol ${createCryptocoinPairDto.baseSymbol}`);
      }
  ...
  async findCoinsPairByBaseSymbolAndQuoteSymbol(basesymbol: string, quotesymbol) {
    const query = {
