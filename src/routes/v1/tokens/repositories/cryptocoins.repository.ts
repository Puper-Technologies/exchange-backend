...
import { CryptocoinDocument, Cryptocoin } from '../schemas/token.schema';
...
export class CryptocoinRepository {
  constructor(
    @InjectModel(Cryptocoin.name)
    private cryptocoinModel: Model<CryptocoinDocument>,
  ...
  public async create(cryptocoin: Partial<Cryptocoin>): Promise<Cryptocoin> {
    try {
      ...
      this.logger.log(
        `Successfully created a new Cryptocoin in db ${newCryptocoin}`,
      );
      return newCryptocoin.toObject();
    } catch (error) {
      this.logger.error(
        `Unexpected error while creating new Cryptocoin ${JSON.stringify(
          cryptocoin,
  ...
  public async addExchangeCurrencies(
    cryptocoin: Partial<Cryptocoin>,
  ): Promise<Cryptocoin> {
    try {
      const newCryptocoin = await this.cryptocoinModel.create({
        ...cryptocoin,
      ...
      this.logger.log(
        `Successfully created a new Cryptocoin in db ${newCryptocoin._id}`,
      );
      return newCryptocoin.toObject();
    } catch (error) {
      this.logger.error(
  ...
  public async getById(id: Types.ObjectId): Promise<Cryptocoin | null> {
    try {
      const cryptocoin = await this.cryptocoinModel.findById(id).lean();

      this.logger.log(
        `Successfully found cryptocoin with id ${cryptocoin._id} from db`,
      );
      return cryptocoin;
    } catch (error) {
      this.logger.error(
