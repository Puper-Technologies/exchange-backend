...
export class CryptocoinsService {
  ...
  async addCryptoCurrency(
    createCryptocoinDto: Partial<TokenEntity>,
  ): Promise<TokenEntity> {
  ...
  async addExchangeCryptoCurrencies(
    ...
    try {
      ...
      const exchangeSupportedCryptoList: any = await lastValueFrom(cryptoAssetList$);
      const cmcCryptoCurrenciesData: any = await lastValueFrom(
      ...
      const cmc_coin_data: any = await lastValueFrom(cmcAssetInfo$);
  ...
  async findAllCryptoCoins(
    ...
    try {
      ...
      const results: TokenEntity[] = await Promise.all(coinsList.paginatedResult.map(async (cryptoCoin): Promise<TokenEntity> => {
  ...
  async findCryptoCoinById(id: Types.ObjectId): Promise<TokenEntity> {
    return await this.cryptocoinRepository.getById(id);
  }

  async findAllCryptoCoinByIds(cryptoCoins: string[]): Promise<TokenEntity[]> {
    return await this.cryptocoinRepository.getByIds(cryptoCoins);
  }

  async findCryptoCoinsBySymbol(symbol: string): Promise<TokenEntity> {
