...
export class UsersRepository {
  ...
  public async create(user: User) {
    try {
      ...
      this.logger.error(
        `Unexpected error while creating new user ${JSON.stringify(
          user,
  ...
  public async getByEmail(email: string): Promise<User | null> {
    try {
      ...
      this.logger.error(
        `Unexpected error while searching user ${email} due to ${error.message}`,
      );
      throw new MongoError(error);
    }
  }
  ...
  public async getVerifiedUserByEmail(email: string): Promise<User | null> {
    try {
      ...
      this.logger.log(
        `Successfully found verified user in db ${foundUser.email}`,
      );
      return foundUser;
    } catch (error) {
      throw new ServiceUnavailableException(
        `Unexpected error while searching verified user ${email} due to ${error.message}`,
      );
    }
  }
  ...
  public async getUnverifiedUserByEmail(email: string): Promise<User | null> {
    try {
      ...
      throw new ServiceUnavailableException(
        `Unexpected error while searching unverified user ${email} due to ${error.message}`,
      );
    }
  }
  ...
  public async getById(id: Types.ObjectId): Promise<User | null> {
    try {
      ...
      throw new ServiceUnavailableException(
        `Unexpected error while searching user with id ${id} due to ${error.message}`,
      );
    }
  }
  ...
  public async getUserByQuery(query: UpdateUserDto): Promise<User | null> {
    try {
      ...
      this.logger.log(
        `Successfully found user by query in db ${JSON.stringify(query)}`,
      );
      return foundUser;
    } catch (error) {
      this.logger.error(
        `Unexpected error while searching user with query ${JSON.stringify(
          query,
        )} due to ${error.message}`,
      );
      throw new MongoError(error);
  ...
  public async updateUserByEmail(
    ...
    try {
      ...
      this.logger.log(
        `Successfully updated the user ${email} with data ${JSON.stringify(
          data,
        )}`,
      );
      return updatedUser;
      ...
      throw new InternalServerErrorException(
        `Unexpected error occured in updating db value due to ${error.message}`,
      );
    }
  }
  ...
  public async getAllUsersWithPagination(
    ...
    const [users, totalCount] = await Promise.all([
      this.usersModel
        ...
        .skip(PaginationUtils.getSkipCount(options.page, options.limit))
        .lean(),
      this.usersModel.count({}).lean(),
    ]);

    return { paginatedResult: users, totalCount };
  }
  async addUserAccountKey(exchange: any, userId: Types.ObjectId) {
    const updated_user = await this.usersModel
      .updateOne({ _id: userId }, { $push: { exchange: exchange } })
      .lean();
    return updated_user;
  }
  ...
  async getUserAccountKey(userId: Types.ObjectId, exchangeName: string) {
    return await this.usersModel
      .find(
        { _id: userId },
        { _id: 0, exchange: { $elemMatch: { exchangeName: exchangeName } } },
      )
      .lean();
  }
}
