...
export class UsersController {
  ...
  async getVerifiedUserById(
    ...
    return ResponseUtil.success('users', foundUser);
  }
  ...
  async getAllUsers(
    ...
    const paginationParams: PaginationParamsInterface | false =
      PaginationUtils.normalizeParams(pageQuery);
    if (!paginationParams) {
      throw new BadRequestException('Invalid pagination parameters');
    }

    const paginatedUsers: PaginatedUsersInterface =
      await this.usersService.getAllUsersWithPagination(paginationParams);
    return ResponseUtil.success('users', paginatedUsers.paginatedResult, {
      location: 'users',
      paginationParams,
      totalCount: paginatedUsers.totalCount,
    });
  ...
  async getUserByMobileNumber(
    @Param('mobNo') mobNo: string,
  ): Promise<UserResponseEntity> {
    const result = (await this.usersService.getUserByMobileNo(
      mobNo,
    )) as UserEntity;
    if (!result)
  ...
  async addUserAccountKey(
    ...
    const result = await this.usersService.addUserBinanceAccountKeys(
      addUserAccountKey,
      exchangeName,
    );
    ...
    return ResponseUtil.success('accountKey', result);
  }
  ...
  async getUserAccountKey(
    ...
    const result = await this.usersService.getUserAccountKey(
      userId,
      exchangeName,
    );

    if (!result) throw new NotFoundException(`Unable to find account key`);
