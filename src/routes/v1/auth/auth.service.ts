...
export class AuthService {
    ...
    try {
      ...
      if (!lodash.isEmpty(user)) {
        throw new NotAcceptableException(
          'User already exist pls try with different credentials',
        );
      }
      ...
      throw new InternalServerErrorException(
        `Unexpected error occured in auth signUp method due to error: ${error.message}`,
      );
    }
  }
  ...
  async login(loginAuthDto: LoginAuthDto) {
    try {
      ...
      const user: UsersEntity = await this.usersService.findByEmail(
        payload.email,
      );
      this.logger.log(`Successfully found user ${user.email}`);
      if (lodash.isEmpty(user)) {
        throw new UnauthorizedException(
          'Unauthorised access pls try with valid user credentials.',
        );
      }

      if (!bcrypt.compare(payload.password, user.password)) {
        throw new UnauthorizedException(
          'Unauthorised access pls try with correct password.',
        );
      }
      ...
      throw new InternalServerErrorException(
        `Unexpected error occured in auth login method due to error: ${error.message}`,
      );
    }
  }
  ...
  async verifyUser(email: string, verify: boolean) {
    ...
    if (!foundUser) {
      throw new NotFoundException(
        'The user is either verified or data not available',
      );
    }

    const user = await this.usersService.update(email, { verified: true });
  ...
  async sendLoginOtp(sendOtpUserDto: SendOtpUserDto) {
    ...
    const user = (await this.usersService.getUserByMobileNo(
      sendOtpUserDto.mobileNo,
    )) as UsersEntity;
    this.logger.log(
      `Successfully found user with mobile no ${sendOtpUserDto.mobileNo}`,
    );
    ...
    this.logger.log(
      `Successfully generated the otp for user ${updatedUser.email}`,
    );
    return otp;
  }
  ...
  async validateOtp(validateOtpUserDto: ValidateOtpUserDto) {
    ...
    if (lodash.isEmpty(user.otp)) {
      throw new NotFoundException(
        'Otp not found for user, please generate new otp',
      );
    }

    if (!lodash.isEqual(user.otp.otpValue, validateOtpUserDto.otpValue)) {
      throw new NotAcceptableException(
        'OTP validation failed, Please try with new otp',
      );
    }
  ...
  async generateAuthToken(jwtpayload: JwtDecodedUser) {
    ...
    await this.usersService.update(payload.email as string, { refreshToken });

    this.logger.log(
      `Successfully generated the token for user the user ${payload.email}`,
    );
    return {
      accessToken,
      refreshToken,
  ...
  public async verifyFirebaseToken(authToken: string): Promise<any | null> {
    try {
      const firebaseUser = await this.firebaseAuthService.authenticateToken(
        authToken,
      );
      return firebaseUser;
    } catch (err) {
  ...
  async logout(token: string) {
    const decodedUser: JwtDecodedUser | null = await this.verifyToken(token);
