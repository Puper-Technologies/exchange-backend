...
export class DoUserNotExist implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const decodedToken = await this.firebaseAuthService.authenticateToken(
      authorization,
    );
    // check whether the user exist with this userId
    const user = await this.userService.getUserByQueryModified({
      authId: decodedToken.uid,
    });
    if (user)
      throw new HttpException(
        { message: 'User already exist with the provided credentials' },
        HttpStatus.BAD_REQUEST,
      );
